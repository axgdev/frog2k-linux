...
static void present_frame(void)
{
	 ...
	 * completed update.  The active descriptor is never rewritten in place.
	 * This is also the exact presentation path used by the physically visible
	 * log78 build.
	 */
	presents++;
	if (presents > 1u)
...
static int condition_native_scanout(void)
{
	...
	/*
	 * Log84 shows that the aggregate GPIO interrupt is not a usable lifetime
	 * TE source on the SF2000: early samples timed out almost one-for-one and
	 * 6,160 of 13,958 retained services eventually waited the full 20 ms for
	 * L08 while holding the shared RGB/8080 pads.  That blanked the panel.  The
	 * physically successful contract is bounded: service the four ownership
	 * edges above, then leave the panel and VOU in RGB mode.
	 */
	panel_te_streaming = 0;
